...
function SceneFigure({ item }: { item: HomeBigDayScene }) {
  return (
    <img
      className={`block w-full rounded-[1.4rem] object-cover shadow-soft${item.aspectClass ? ` ${item.aspectClass}` : ""
        }`}
      src={item.src}
      alt={item.alt}
      width={item.width}
    ...
    <figure className="home-guestbook-stack m-0">
      {item.photo ? (
        <div className="home-guestbook-polaroid">
          <img src={item.photo.src} alt={item.photo.alt} width={1024} height={1536} loading="lazy" />
        </div>
      ) : null}
      <blockquote
      ...
      >
        ...
        <footer className="home-guestbook-attribution">— {item.attribution}</footer>
      </blockquote>
    </figure>
  );
}
  ...
  return (
    <div className="home-big-day-mosaic mx-auto max-w-[1480px]">
      {HOME_ON_THE_BIG_DAY.items.map((item, index) => (
        ...
        >
          {item.kind === "scene" ? <SceneFigure item={item} /> : <GuestbookCard item={item} />}
        </div>
      ))}
    </div>
  );
  ...
  return (
    <figure className="m-0">
      <img
        ...
        loading="lazy"
      />
      <figcaption className="mt-3 font-serif text-[1.05rem] text-ink-soft">{moment.caption}</figcaption>
    </figure>
  );
}
...
function WhatYouGetFeatures({ titleAs: Title = "h2" }: { titleAs?: "h2" | "h3" }) {
  return (
    <ul className="m-0 grid list-none gap-8 p-0 min-[720px]:grid-cols-2 min-[720px]:gap-x-0 min-[720px]:gap-y-8 min-[1100px]:grid-cols-4 min-[1100px]:gap-0">
      {HOME_FEATURES.map((feature, index) => (
        <li
          key={feature.title}
          className={`flex gap-4 px-1 min-[720px]:px-4 min-[1100px]:px-5 ${index % 2 === 1
...
function OccasionCard({ occasion, className }: { occasion: HomeOccasion; className?: string }) {
  return (
    <Link
