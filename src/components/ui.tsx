...
function SiteFooter({
  ...
  return (
    <footer className='mt-auto border-t border-[color-mix(in_srgb,var(--color-ink)_12%,transparent)] px-[6vw] py-8 print:hidden'>
      <div
        className={`mx-auto flex ${maxWidthClass} flex-col gap-5 text-[0.85rem] text-ink-soft`}
      >
        {showLogo ? (
          <Link
            className='mx-auto inline-flex items-center no-underline'
            href='/'
          >
            <img
              src='/images/logo.png'
...
export function Field({ label, hint, children }: FieldProps) {
  return (
    <label className='block'>
      ...
      {children}
      {hint ? (
        <span className='mt-1.5 block text-[0.85rem] text-ink-soft'>
          {hint}
        </span>
      ) : null}
    </label>
