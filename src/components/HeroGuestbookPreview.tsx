    ...
    (async () => {
      ...
      while (!signal.cancelled) {
        ...
        for (let i = 0; i < entries.length; i += 1) {
          ...
          const previewNextAt =
            i < entries.length - 1
              ? Math.max(0, attrMs - NEXT_FADE_LEAD_MS)
              : null;
          let typedMs = 0;
...
function TypedEntry({
  ...
  const liClassName = `relative overflow-hidden rounded-[0.4rem] border border-[color-mix(in_srgb,var(--color-ink)_11%,transparent)] bg-[color-mix(in_srgb,var(--color-paper)_70%,white)] ${
    index === 1 ? 'ml-3' : ''
  }`;
        ...
        <div className='flex items-start gap-3 py-2.5 pl-3.5 pr-3'>
          ...
          <blockquote className='relative m-0 min-w-0 pt-0.5'>
            ...
            <div className='absolute inset-0 pt-0.5'>
              ...
              {showAttr ? (
                <footer className={footerClass}>{attrText}</footer>
              ) : null}
            </div>
          </blockquote>
        </div>
  ...
  return (
    <motion.li
      ...
      transition={{
        duration: staticMode ? 0 : FADE_MS / 1000,
        ease: 'easeOut',
      }}
...
export function HeroGuestbookPreview() {
  ...
  const { cycleIndex, progress, setOpacity, reducedMotion } =
    useHeroGuestbookSequence(HOME_HERO_GUESTBOOK.cycles, prefersReducedMotion);

  const entries =
    HOME_HERO_GUESTBOOK.cycles[reducedMotion ? 0 : cycleIndex] ??
    HOME_HERO_GUESTBOOK.cycles[0];
  const sizingEntries =
    HOME_HERO_GUESTBOOK.cycles[SIZING_CYCLE_INDEX] ??
    HOME_HERO_GUESTBOOK.cycles[0];
    ...
    <div className='relative mx-auto w-full max-w-[26rem] min-[900px]:max-w-none'>
      ...
      <figure className='relative m-0'>
          ...
          <div className='rounded-[1rem] border border-[color-mix(in_srgb,var(--color-ink)_9%,transparent)] px-3.5 pb-4 pt-3.5 min-[900px]:px-4 min-[900px]:pb-4 min-[900px]:pt-4'>
            ...
            <div className='relative'>
              ...
              <motion.ul
                ...
                transition={{
                  duration: reducedMotion ? 0 : FADE_MS / 1000,
                  ease: 'easeOut',
                }}
                aria-hidden='true'
              >
        ...
        <figcaption className='sr-only'>
          A sample Willow Book guestbook with notes and a guest photo.
        </figcaption>
      </figure>
    </div>
  );
