...
function VisualStory({
  ...
  story: NonNullable<MarketingContent['visualStory']>;
}) {
  return (
...
function Highlights({
  ...
  highlights: NonNullable<MarketingContent['highlights']>;
}) {
  return (
    <section className='mt-16'>
      <div className='grid gap-4 min-[700px]:grid-cols-2'>
        {highlights.map((item) => (
          ...
          >
            <h2 className='mb-2 font-serif text-[1.2rem] font-medium'>
              {item.title}
            </h2>
    ...
    <>
      <section className='max-w-[760px] pb-16 pt-8'>
        ...
        <div className={btnRowClass}>
          ...
          <a
            className={btnClass('ghost')}
            href={eventGuestbookPath(content.demoSlug)}
          >
      ...
      <ol className='mt-8 grid list-none gap-4 p-0 min-[700px]:grid-cols-3'>
        {content.steps.map((step, index) => (
          ...
          >
            <span className='font-serif text-[1.4rem] text-accent'>
              {index + 1}
            </span>
            <h2>{step.title}</h2>
