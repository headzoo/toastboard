...
const transitions = {
  ...
  swing: {
    initial: { opacity: 0, rotate: -1.2 },
    animate: { opacity: 1, rotate: 0 },
    exit: { opacity: 0, rotate: 1.2 },
  },
} as const;
...
function transitionFor(
  ...
  return transitions[
    preference === 'random'
      ? styles[stableNumber(slotKey) % styles.length]
      : preference
  ];
}
...
export function GuestbookSlideshow({
  ...
  const [videoFailedCycleKey, setVideoFailedCycleKey] = useState<string | null>(
    null,
  );
  const reducedMotion = useReducedMotion();
  const expanded =
    expandState?.cycleKey === deck.slot.cycleKey ? expandState.target : null;
  ...
  const toggleExpand = (target: ExpandTarget) => {
    setExpandState((current) => {
      if (current?.cycleKey === deck.slot.cycleKey && current.target === target)
        return null;
  ...
  const handleVideoError = useCallback(() => {
    ...
    setExpandState((current) =>
      current?.cycleKey === deck.slot.cycleKey && current.target === 'video'
        ? null
        : current,
    );
  }, [deck.slot.cycleKey]);
  ...
  const animation = transitionFor(
    deck.slot.cycleKey,
    deck.preferences.motion,
    Boolean(reducedMotion),
  );
  const hasPhoto =
    deck.slot.kind === 'content' && deck.slot.media.kind === 'photo';
  const hasVideo =
    deck.slot.kind === 'content' && deck.slot.media.kind === 'video';
  ...
  const guestLabel =
    deck.slot.kind === 'content' ? deck.slot.guestName || 'A guest' : 'A guest';
  ...
  return (
    <section
      ...
      style={rootStyle}
      aria-label={`${event.coupleNames} slideshow`}
    >
      <div
            ...
            >
              {poster ? (
                <img
                  ...
                  src={poster}
                  alt={`Table sign for ${event.coupleNames}`}
                />
              ) : null}
            </motion.div>
          ) : (
            ...
            >
              {deck.slot.media.kind === 'photo' ? (
                <button
                  ...
                  aria-label={
                    expanded === 'photo' ? 'Close photo' : 'Expand photo'
                  }
                  ...
                  <div className='slideshow-photo-frame'>
                    <img
                      ...
                      className={
                        ...
                        deck.slot.dimensions.height > deck.slot.dimensions.width
                ...
                <div className='slideshow-video-frame'>
                  {videoFailed ? (
                    <div className='slideshow-video-unavailable' role='status'>
                      <p className='slideshow-video-unavailable-text'>
                        Video unavailable
                      </p>
                    </div>
                  ) : (
                    <>
                      ...
                      <button
                        ...
                        aria-label={
                          expanded === 'video' ? 'Close video' : 'Expand video'
                        }
                      >
                        Expand
                      </button>
      ...
      <div className='slideshow-controls'>
        <button
          ref={gearRef}
          className='slideshow-gear'
          type='button'
          onClick={() => setSettingsOpen(true)}
          aria-label='Open slideshow settings'
        >
          ...
          >
            ...
            {expanded === 'photo' && deck.slot.media.kind === 'photo' ? (
              ...
              >
                <div className='slideshow-expand-photo-frame'>
                  <img
                    className='slideshow-expand-photo-image'
                    src={deck.slot.media.url}
                    alt=''
                  />
                </div>
            ...
            !videoFailed ? (
              ...
              >
                ...
                <button
                  type='button'
                  className='slideshow-expand-video-close'
                  onClick={() => setExpandState(null)}
                >
                  Close video
                </button>
