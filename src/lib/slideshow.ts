...
export type SlideMediaTarget =
  TextMediaTarget | PhotoMediaTarget | VideoMediaTarget;
...
export function isCurrentDeckCandidate(
  ...
  if (media.kind === 'text')
    return (
      Boolean(liveEntry.text) &&
      !liveEntry.photoUrls.length &&
      !liveEntry.videoUrl
    );
  if (media.kind === 'photo')
    return (
      liveEntry.photoUrls[media.index] === media.url && !liveEntry.videoUrl
    );
...
function isDuration(value: unknown): value is SlideshowDuration {
  return (
    typeof value === 'number' &&
    SLIDESHOW_DURATIONS.includes(value as SlideshowDuration)
  );
}

function isMotionStyle(value: unknown): value is MotionStyle {
  return (
    typeof value === 'string' && MOTION_STYLES.includes(value as MotionStyle)
  );
}

export function validateSlideshowPreferences(
  value: unknown,
...
export function loadSlideshowPreferences(slug: string): SlideshowPreferences {
  try {
    ...
    return saved
      ? validateSlideshowPreferences(JSON.parse(saved))
...
export function saveSlideshowPreferences(
  slug: string,
  ...
  try {
    window.localStorage.setItem(
      preferenceKey(slug),
      JSON.stringify(validateSlideshowPreferences(preferences)),
    );
  } catch {
...
export function toDeckEntry(message: MessageRecord): DeckEntry | null {
  const videoUrl =
    message.videoStatus === 'ready' ? message.videoUrl?.trim() || null : null;
...
export function mediaTargets(entry: DeckEntry): readonly SlideMediaTarget[] {
  ...
  if (entry.photoUrls.length)
    return entry.photoUrls.map((url, index) => ({ kind: 'photo', index, url }));
...
export function shuffleEntries(
  ...
  if (avoidFirstId && shuffled.length > 1 && shuffled[0]?.id === avoidFirstId) {
    const alternative = shuffled.findIndex(
      (entry) => entry.id !== avoidFirstId,
    );
    if (alternative > 0)
      [shuffled[0], shuffled[alternative]] = [
