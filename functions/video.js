...
function parseRawMediaCandidate(name) {
  ...
  if (!match) return null;
  return { slug: match[1], messageId: match[2], extension: match[3].toLowerCase() };
}

function isValidRawMedia(metadata, media) {
  if (!media || !SLUG_PATTERN.test(media.slug) || !MESSAGE_ID_PATTERN.test(media.messageId)) {
    return false;
  }
...
function inspectProbeOutput(output) {
  ...
  const videoMatch = /^\s*Stream #\d+:\d+(?:\[[^\]]+\])?(?:\([^)]+\))?: Video:\s*([^,\s]+)/m.exec(text);
  const audioMatch = /^\s*Stream #\d+:\d+(?:\[[^\]]+\])?(?:\([^)]+\))?: Audio:\s*([^,\s]+)/m.exec(text);
  return {
    container,
...
function isRemuxEligible(probe) {
  const mp4Family = /(?:^|,)\s*(mov|mp4|m4a|3gp|3g2|mj2)\s*(?:,|$)/.test(probe?.container ?? "");
  return Boolean(
    mp4Family &&
...
function buildDownloadUrl({ bucket, objectPath, token, emulatorHost = process.env.FIREBASE_STORAGE_EMULATOR_HOST }) {
...
function isVisibleProcessing(data) {
  return Boolean(data && data.isHidden !== true && data.videoStatus === "processing");
}

function terminalVideoState(data) {
  if (!data) return "missing";
  if (data.isHidden === true) return "hidden";
  if (data.videoStatus === "ready" || data.videoStatus === "failed") return data.videoStatus;
  return "other";
}
...
function videoReadyTransitionOutcome(data, videoUrl) {
  ...
  if (data.videoStatus === "failed") return { status: "rejected", reason: "failed" };
  if (data.videoStatus !== "processing") return { status: "rejected", reason: "ineligible" };
...
function runFfmpeg(binary, args, { timeoutMs = 105000, allowNonZero = false } = {}) {
