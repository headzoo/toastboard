...
test('parses only exact raw video paths', () => {
  ...
  assert.equal(
    parseRawMediaPath(`events/${slug}/messages/${messageId}.mp4`),
    null,
  );
  assert.equal(
    parseRawMediaPath(`events/${slug}/messages/${messageId}-raw.MP4`),
    null,
  );
  assert.equal(
    parseRawMediaPath(`events/${slug}/messages/not-a-uuid-raw.mp4`),
    null,
  );
});

test('rejects empty and extension/MIME-mismatched raw uploads', () => {
  const media = parseRawMediaPath(
    `events/${slug}/messages/${messageId}-raw.mp4`,
  );
  assert.equal(
    isValidRawMedia({ size: '0', contentType: 'video/mp4' }, media),
    false,
  );
  assert.equal(
    isValidRawMedia({ size: '1024', contentType: 'video/webm' }, media),
    false,
  );
  assert.equal(
    isValidRawMedia({ size: '10485760', contentType: 'video/mp4' }, media),
    false,
  );
  assert.equal(
    isValidRawMedia({ size: '1024', contentType: 'video/mp4' }, media),
    true,
  );
});
...
`);
  ...
  assert.equal(
    isRemuxEligible({ ...h264Aac, container: 'matroska,webm' }),
    false,
  );
});
